...
{
       printk(KERN_INFO "Aruna's char reading...\n");
}

void  my_write(struct file *f,char __user *buffer, size_t length, loff_t *off)
{
       printk(KERN_INFO "Aruna's char writing...\n");
}
...
{
	printk(KERN_INFO "Aruna's char registered\n");
        
	if(alloc_chrdev_region(&first,0,10,"Aruna's Assignment1")<0)
	{
		return -1;
	}
         printk(KERN_INFO "Major=%d\n",MAJOR(first));
	if ((cls=class_create(THIS_MODULE,"Aruna's chardrv"))==NULL)
	{
...
static void __exit mychar_exit(void)
{
	...
	class_destroy(cls);
	unregister_chrdev_region(first,10);
	printk(KERN_INFO "Aruna's char unregistered\n");
}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Aruna Anantharaj");
